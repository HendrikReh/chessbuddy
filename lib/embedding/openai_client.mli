(** OpenAI API client for text embeddings.
    ...
    Low-level HTTP client for OpenAI's text-embedding API. Handles
...
(** [create ?api_key ?model ?endpoint ()] initializes an OpenAI client.
    ...
    @param model
      Model name. Defaults to ["gpt-5"]. For embeddings use
      ["text-embedding-3-small"] or ["text-embedding-3-large"].
    @param endpoint
      API endpoint URI. Defaults to [https://api.openai.com/v1/embeddings].
...
(** [embed client text] generates a 1536-dimensional embedding vector.
    ...
    {[
      ...
      | Error msg -> Printf.eprintf "Embedding failed: %s\n" msg
    ]}

    Note: The default model parameter in {!create} is ["gpt-5"] which is
    incorrect for embeddings. Always specify [~model:"text-embedding-3-small"]
    or similar. *)

(** {1 Internal Functions} *)

val fetch_api_key : unit -> (string, string) Result.t
(** [fetch_api_key ()] retrieves API key from [OPENAI_API_KEY] environment
    variable.
