...
let test_upsert_player_with_fide () =
  Alcotest_lwt.test_case "upsert player with FIDE ID" `Quick (fun _switch () ->
      with_clean_db (fun pool ->
          ...
          let%lwt result2 =
            Chessbuddy.Database.upsert_player pool
              ~full_name:"Magnus Carlsen (Updated)"
              ~fide_id:(Some "1503014")
          in
          ...
          Alcotest.(check uuid_testable)
            "Same player ID for same FIDE ID"
            player_id player_id2;
          Lwt.return_unit))

(* Test player upsert without FIDE ID *)
let test_upsert_player_without_fide () =
  Alcotest_lwt.test_case "upsert player without FIDE ID" `Quick (fun _switch () ->
      with_clean_db (fun pool ->
          let%lwt result =
            Chessbuddy.Database.upsert_player pool
          ...
          let%lwt result2 =
            Chessbuddy.Database.upsert_player pool
              ~full_name:"UNKNOWN PLAYER"  (* Different case *)
              ~fide_id:None
          in
          let player_id2 = check_ok "Second upsert failed" result2 in
          ...
          Alcotest.(check uuid_testable)
            "Same player ID for normalized name match"
            player_id player_id2;
          Lwt.return_unit))
...
let test_record_rating () =
  Alcotest_lwt.test_case "record player rating" `Quick (fun _switch () ->
      with_clean_db (fun pool ->
          ...
          let%lwt result =
            Chessbuddy.Database.record_rating pool
              ~player_id
              ~date:(Ptime.to_date date)
              ~standard:2800
              ~rapid:2750
              ()
          in
...
let test_create_batch () =
  Alcotest_lwt.test_case "create ingestion batch" `Quick (fun _switch () ->
      with_clean_db (fun pool ->
          ...
          let%lwt result2 =
            Chessbuddy.Database.create_batch pool
              ~source_path:"/path/to/test2.pgn"
              ~label:"test-batch-updated"
              ~checksum:"abc123"
          in
          let batch_id2 = check_ok "Batch re-creation failed" result2 in

          Alcotest.(check uuid_testable)
            "Same batch ID for same checksum"
            batch_id batch_id2;
          Lwt.return_unit))
...
let test_upsert_fen () =
  Alcotest_lwt.test_case "upsert FEN position" `Quick (fun _switch () ->
      with_clean_db (fun pool ->
          ...
          Alcotest.(check uuid_testable)
            "Same FEN ID for identical position"
            fen_id fen_id2;
          Lwt.return_unit))
...
let test_record_game () =
  Alcotest_lwt.test_case "record game" `Quick (fun _switch () ->
      with_clean_db (fun pool ->
          ...
          (* Record game *)
          let%lwt result =
            Chessbuddy.Database.record_game pool
              ~white_id
              ~black_id
              ~header
              ~source_pgn:"1. e4 e5 2. Nf3 1-0"
              ~batch_id
          in
