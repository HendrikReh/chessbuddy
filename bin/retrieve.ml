...
let uri_conv : Uri.t Arg.conv =
  let parse s =
    ...
    with exn ->
      let msg = Fmt.asprintf "invalid URI %S (%s)" s (Stdlib.Printexc.to_string exn) in
      Error (`Msg msg)
  in
...
let format_exn = function Failure msg -> msg | exn -> Stdlib.Printexc.to_string exn
...
let print_fen_info (info : Database.fen_info) =
  ...
  | Some version ->
      ...
      Option.iter info.embedding ~f:(fun emb ->
          let preview = if String.length emb > 80 then String.prefix emb 80 ^ "…" else emb in
    ...
    Ingestion_pipeline.with_pool uri (fun pool ->
        ...
        | Some base_fen ->
            ...
            | Some _ ->
                ...
                let* sims_res =
                  Database.find_similar_fens pool ~fen_id:base_fen.fen_id ~limit:sample_limit
                in
                let* sims = fail_on_error sims_res in
                let filtered =
                  List.filter sims ~f:(fun s -> not (Uuidm.equal s.Database.fen_id base_fen.fen_id))
                in
                (match filtered with
                ...
                | _ ->
                    ...
                    List.iteri filtered ~f:(fun idx sim ->
                        Fmt.printf "  %d. %s (distance %.4f, usage %d, version %s)@."
                          (idx + 1)
                          sim.Database.fen_text
                          sim.Database.distance
  ...
  let action () =
    Ingestion_pipeline.with_pool uri (fun pool ->
        ...
        | Some detail ->
             ...
             Fmt.printf "%s vs %s (%s)@."
               header.white_player header.black_player header.result;
             Option.iter header.event ~f:(fun event -> Fmt.printf "  Event: %s@." event);
             Option.iter header.site ~f:(fun site -> Fmt.printf "  Site: %s@." site);
             Option.iter header.game_date ~f:(fun date ->
                 Fmt.printf "  Date: %s@." (pp_timestamp date));
             Option.iter header.round ~f:(fun round -> Fmt.printf "  Round: %s@." round);
             Option.iter header.eco ~f:(fun eco -> Fmt.printf "  ECO: %s@." eco);
             Option.iter header.opening ~f:(fun opening -> Fmt.printf "  Opening: %s@." opening);
             ...
             | _ -> ());
             Option.iter header.white_fide_id ~f:(fun fid -> Fmt.printf "  White FIDE: %s@." fid);
             Option.iter header.black_fide_id ~f:(fun fid -> Fmt.printf "  Black FIDE: %s@." fid);
             Option.iter header.termination ~f:(fun term -> Fmt.printf "  Termination: %s@." term);
             Option.iter detail.batch_label ~f:(fun label -> Fmt.printf "  Batch: %s@." label);
             Fmt.printf "  Ingested: %s@." (pp_timestamp detail.ingested_at);
             Fmt.printf "  Moves stored: %d@." detail.move_count;
             (if show_pgn then
                Fmt.printf "@[%s@]@." detail.source_pgn
              else
                Fmt.printf "  (use --pgn to print full PGN record)@.");
             Lwt.return_unit)
        ))
  in
  run_lwt action
  ...
  let action () =
    Ingestion_pipeline.with_pool uri (fun pool ->
        let print_summary summary =
          Fmt.printf "Batch %s@." (Uuidm.to_string summary.Database.overview.batch_id);
          ...
          Fmt.printf "  Ingested at: %s@." (pp_timestamp summary.Database.overview.ingested_at);
        ...
        | None, Some label_text ->
            let* batches_res = Database.find_batches_by_label pool ~label:label_text ~limit in
            ...
            let rec iterate = function
              ...
              | overview :: rest ->
                  let* summary_res =
                    Database.get_batch_summary pool ~batch_id:overview.Database.batch_id
                  in
                  let* summary_opt = fail_on_error summary_res in
                  let* () =
                    match summary_opt with
        ...
        | Some info ->
            ...
            let json =
              `Assoc
                [
                  ( "fen_id",
                    `String (Uuidm.to_string info.Database.fen_id) );
                  ("fen_text", `String info.fen_text);
                  ("side_to_move", `String (String.of_char info.side_to_move));
                  ("castling", `String info.castling);
                  ( "en_passant",
                    match info.en_passant with None -> `Null | Some ep -> `String ep );
                  ("material_signature", `String info.material_signature);
                  ( "embedding_version",
                    match info.embedding_version with None -> `Null | Some v -> `String v );
                  ( "embedding",
                    match info.embedding with None -> `Null | Some e -> `String e );
                  ...
                  ( "similar",
                    `List
                      (List.map sims ~f:(fun sim ->
                           `Assoc
                             [
                               ("fen_id", `String (Uuidm.to_string sim.Database.fen_id));
                               ("fen_text", `String sim.fen_text);
                               ("distance", `Float sim.distance);
                               ("usage_count", `Int sim.usage_count);
                               ("embedding_version", `String sim.embedding_version);
                             ])) );
                ]
            in
...
let similar_cmd =
  ...
  let db_uri =
    Arg.(required & opt (some uri_conv) None & info [ "db-uri" ] ~doc:"PostgreSQL connection URI")
  in
  let fen = Arg.(required & opt (some string) None & info [ "fen" ] ~doc:"FEN text to search for") in
  let limit =
    Arg.(value & opt int 5 & info [ "k" ] ~doc:"Number of neighbours to display" ~docv:"K")
  in
...
let game_cmd =
  ...
  in
  let show_pgn = Arg.(value & flag & info [ "pgn" ] ~doc:"Print full PGN source") in
...
let player_cmd =
  ...
  in
  let player_name =
    Arg.(required & opt (some string) None & info [ "name" ] ~doc:"Player name substring")
  in
  let limit =
...
let batch_cmd =
  ...
  let db_uri =
    Arg.(required & opt (some uri_conv) None & info [ "db-uri" ] ~doc:"PostgreSQL connection URI")
  in
  let id = Arg.(value & opt (some uuid_conv) None & info [ "id" ] ~doc:"Batch UUID") in
  let label = Arg.(value & opt (some string) None & info [ "label" ] ~doc:"Batch label filter") in
  let limit =
    Arg.(value & opt int 5 & info [ "limit" ] ~doc:"Maximum label matches to summarize")
  in
...
let export_cmd =
  ...
  in
  let fen_id =
    Arg.(required & opt (some uuid_conv) None & info [ "id" ] ~doc:"FEN UUID to export")
  in
  let output =
    Arg.(required & opt (some file) None & info [ "out" ] ~doc:"Destination JSON file")
  in
  let limit =
    Arg.(value & opt int 5 & info [ "k" ] ~doc:"Similar FENs to include" ~docv:"K")
  in
  let term = Term.(ret (const export_fen_action $ db_uri $ fen_id $ output $ limit)) in
  Cmd.v info term

let commands =
  [ similar_cmd; game_cmd; fen_cmd; player_cmd; batch_cmd; export_cmd ]

let () = Stdlib.exit (Cmd.eval (Cmd.group (Cmd.info "retrieve" ~doc:"Chess retrieval CLI") commands))
