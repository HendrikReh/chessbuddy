(** Natural language search service for ChessBuddy.
    ...
    This module provides semantic search over indexed chess entities (games,
    players, FENs, batches, embeddings) using text embeddings and vector
    similarity. It handles query embedding, entity filtering, and result
    ranking.
...
(** [resolve_entity_filter raw] normalizes and resolves an entity type name.
    ...
    Accepts case-insensitive input with whitespace (e.g., [" Game "] →
    ["game"]).
...
(** [search pool ~embedder ~query ~entity_filters ~limit] performs natural
    language search.

    Workflow: 1. Validate query is non-empty (stripped) 2. Clamp limit to
    [1, 200] range 3. Ensure [search_documents] table exists 4. Embed query text
    to 1536-dimensional vector 5. Execute vector similarity search filtered by
    entity types 6. Return ranked results with relevance scores
    ...
    Returns: List of {!Database.search_hit} ordered by similarity (descending
    score)
    ...
    {[
      ...
      let embedder =
        (module Search_embedder.OpenAI : Search_indexer.TEXT_EMBEDDER)
      in
      let%lwt hits =
