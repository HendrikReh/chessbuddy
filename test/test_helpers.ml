...
let exec_query pool query params =
  ...
  in
  match result with Ok () -> Lwt.return_unit | Error err -> raise_or_skip err

(* Clean up test data from all tables *)
let cleanup_test_data pool =
  let open Caqti_request.Infix in
  let delete_games_positions =
    (Caqti_type.unit -->. Caqti_type.unit) @:- "DELETE FROM games_positions"
  in
  ...
  in
  let delete_fen_embeddings =
    (Caqti_type.unit -->. Caqti_type.unit) @:- "DELETE FROM fen_embeddings"
  in
  let delete_fens =
    (Caqti_type.unit -->. Caqti_type.unit) @:- "DELETE FROM fens"
  in
  let delete_ingestion_batches =
    (Caqti_type.unit -->. Caqti_type.unit) @:- "DELETE FROM ingestion_batches"
  in
  let delete_player_ratings =
    (Caqti_type.unit -->. Caqti_type.unit) @:- "DELETE FROM player_ratings"
  in
  let delete_players =
...
let with_clean_db f =
  Lwt.catch
    (fun () ->
      ...
      Lwt.finalize (fun () -> f pool) (fun () -> cleanup_test_data pool))
    (function
