...
module type EMBEDDER = sig
  val version : string
  (** Embedding model version identifier (e.g., ["v1"], ["openai-text-embedding-3-small"]) *)
...
(** FEN position embedder interface.
    ...
    Implementations may use neural networks, hand-crafted features, or placeholder values.
    See {!Embedder.Constant} for a stub implementation. *)
...
module type PGN_SOURCE = sig
  ...
  (** [fold_games path ~init ~f] streams games from a PGN file.
      ...
      Implementations should handle multi-game PGN files, preserve move annotations
      (comments, variations, NAGs), and detect game boundaries correctly. *)
end
...
val record_player :
  ...
  Uuidm.t Lwt.t
(** [record_player pool ~name ~fide_id ~search_embedder] upserts a player and indexes for search.

    - Deduplicates by FIDE ID (if present) or normalized name
    - Indexes player text for natural language search (when [search_embedder] provided)
...
val sync_players_from_pgn :
  ...
  pgn_path:string ->
  int Lwt.t
(** [sync_players_from_pgn source pool ~pgn_path] extracts unique players without storing games.
...
(** [material_signature fen] computes a piece-count fingerprint from FEN board state.
...
(** [fen_components fen] extracts [(side_to_move, castling_rights, en_passant)] from FEN.
...
(** [motifs_for_move move] extracts tactical motifs from move features.
    ...
    Currently returns [[]] (placeholder). Future versions will detect pins, forks, etc. *)
...
val process_move :
  ...
  search_embedder:(module TEXT_EMBEDDER) option ->
  unit Lwt.t
(** [process_move pool ~game_id ~embedder ~move ~search_embedder] persists a single move.

    Workflow:
    1. Extract FEN components (side to move, castling, en passant)
    2. Compute material signature
    3. Upsert FEN position (deduplicates by FEN text)
    4. Record position in [games_positions] table
    5. Generate and store embedding if missing or version changed
    6. Index FEN for natural language search
...
val process_game :
  ...
  search_embedder:(module TEXT_EMBEDDER) option ->
  unit Lwt.t
(** [process_game pool ~embedder ~batch_id ~game ~source_path ~batch_label ~search_embedder]
    ingests a single game.

    Workflow:
    1. Upsert white and black players
    2. Record game metadata
    3. Process each move sequentially (see {!process_move})
...
(** [ingest_file source pool ~embedder ~pgn_path ~batch_label ~search_embedder ()]
    ...
    Workflow:
    1. Ensure [ingestion_batches] and [search_documents] tables exist
    2. Compute PGN file checksum
    3. Create or retrieve batch (deduplicates by checksum)
    4. Stream games from PGN file
    5. Process each game (players, positions, embeddings)

    Example:
    {[
      let pool = Database.Pool.create (Uri.of_string db_uri) in
      let embedder = (module Embedder.Constant : EMBEDDER) in
      let search_embedder = Some (module Search_embedder.Stub : TEXT_EMBEDDER) in
      let%lwt () =
        ingest_file (module Pgn_source) pool ~embedder
          ~pgn_path:"data/games/twic1611.pgn"
          ~batch_label:"twic-1611"
          ~search_embedder ()
      in
      Lwt.return_unit
    ]}
...
(** [inspect_file source ~pgn_path] performs dry-run analysis without database writes.

    Returns summary with game count, move count, and unique player list.
    Useful for previewing PGN contents before ingestion. *)
